...
int forceMountOn = false;
int forceParkPinOn = false;
...
SerialCommand myDeviceCmd;

void setup()
{
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  // Pin mode and initil setting
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  accelGatPark[0] = (double)  0;  // Stores the real accel value in g's
  accelGatPark[1] = (double)  - 1; // Stores the real accel value in g's
  accelGatPark[2] = (double) - 0; // Stores the real accel value in g's
  double len;
  ...
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  myDeviceCmd.addCommand("getAll", getAll);
  myDeviceCmd.addCommand("IDENT", Cmd2);
  myDeviceCmd.addCommand("StreamOn", Cmd3);
  myDeviceCmd.addCommand("StreamOff", Cmd4);
  myDeviceCmd.addCommand("isParked", Cmd5);
  myDeviceCmd.addCommand("FrcMntOn",    ForceMountOn);
  myDeviceCmd.addCommand("FrcMntOff",   ForceMountOff);
  myDeviceCmd.addCommand("FrcPPOn",     ForceParkPinOn);
  myDeviceCmd.addCommand("FrcPPOff",    ForceParkPinOff);
...
void getAll()
{
  ...
  if (forceParkPinOn) {
    Serial.print("ForceParkPinOn=True, ");
  }      else {
    Serial.print("ForceParkPinOn=Flase,");
  }
  
  if (forceMountOn) {
    Serial.print("ForceMountOn=Ture\t" );
  } else {
    Serial.print("ForceMountOn=Flase\t" );
  }
  Serial.println();
}
...
void ForceParkPinOff()
{
  forceParkPinOn=false;
}
